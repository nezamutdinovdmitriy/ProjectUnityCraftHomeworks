...
class GameObjectPool
{
    ...
    public T Rent<T>(GameObject prefab) where T : Component => this.Rent(prefab).GetComponent<T>();

    public GameObject Rent(GameObject prefab)
    {
        if (!_pool.TryGetValue(prefab.name, out Stack<GameObject> stack))
        {
            stack = new Stack<GameObject>();
            _pool.Add(prefab.name, stack);
        }
    ...
    {
        if (_pool.TryGetValue(go.name, out Stack<GameObject> stack)) 
            stack.Push(go);
    }
}
