...
using UnityEngine;
using static Modules.AI.InternalUtils;
...
namespace Modules.AI
{
    public partial class Blackboard
    {
        ...
        /// <summary>
        /// Invoked when a value is deleted from the blackboard.
        /// </summary>
        public event Action<int> OnValueDeleted;

        /// <summary>
        /// Invoked when a value is changed in the blackboard.
        /// </summary>
        public event Action<int> OnValueChanged;

        /// <summary>
        /// Gets the total number of values stored in the blackboard.
        /// </summary>
        ...
        /// <exception cref="KeyNotFoundException">Thrown if the key does not exist in the blackboard.</exception>
        ...
        public bool TryGetValue<T>(int key, out T value)
        {
            if (_valueCount > 0)
            {
                ...
                while (index >= 0)
                {
                    ...
                    if (slot.exists && slot.key == key)
                    {
                        object stored = slot.value;

                        if (slot.primitive)
                        {
                            if (stored is Boxing<T> boxed)
                            {
                                value = boxed.value;
                                return true;
                            }
                        ...
                        throw new InvalidCastException(
                            $"Blackboard key {key}: stored type is {stored?.GetType()}, requested {typeof(T)}");
                    }
        ...
        /// <summary>
        /// Checks whether the blackboard contains a value with the specified key.
        /// </summary>
        ...
        /// <summary>
        /// Adds a reference type value to the blackboard.
        /// </summary>
        ...
        /// <summary>
        public void AddReferenceValue<T>(int key, T value) where T : class
        {
            if (value == null)
                throw new ArgumentNullException(nameof(value), $"Parameter with key {BlackboardKeys.IdToName(key)} is null");
            ...
            this.OnValueAdded?.Invoke(key);
            this.OnStateChanged?.Invoke();
        }

        public void AddPrimitiveValue<T>(int key, T value) where T : struct
        {
            int hash, bucket, index;
        ...
        /// <summary>
        /// Deletes a value by key from the blackboard.
        /// </summary>
            ...
            if (_valueCount > 0)
            {
                ...
                while (index >= 0)
                {
                    ...
                    if (node.key == key)
                    {
                        ...
                        this.OnValueDeleted?.Invoke(key);
                        this.OnStateChanged?.Invoke();
                        return true;
                    }
        ...
        /// <exception cref="ArgumentNullException">Thrown if the value is null.</exception>
        public void SetReferenceValue(int key, object value)
        {
            if (value == null)
        ...
        /// <param name="key">The key associated with the value.</param>
        /// <param name="value">The new value.</param>
        public void SetPrimitiveValue<T>(int key, T value) where T : struct
        {
            if (this.FindValueIndex(key, out int index))
            {
                ref ValueSlot slot = ref _valueSlots[index];
        ...
        public void ClearValues()
        {
            ...
            for (int i = 0; i < removedCount; i++)
                this.OnValueDeleted?.Invoke(removedItems[i]);
        }

        /// <summary>
        /// Returns an array of all key-value pairs stored in the blackboard.
        /// </summary>
        ...
        {
            this.OnValueChanged?.Invoke(key);
            this.OnStateChanged?.Invoke();
        }
        ...
        private KeyNotFoundException ValueNotFoundException(int key) =>
            new($"The given value {BlackboardKeys.IdToName(key)} was not present in the blackboard: {name}");

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private Exception ValueAlreadyAddedException(int key) =>
            new ArgumentException(
                $"A value with the same key {BlackboardKeys.IdToName(key)} already has been added in the blackboard: {name}!");
        ...
        public struct ValueEnumerator : IEnumerator<KeyValuePair<int, object>>
        {
            ...
            public ValueEnumerator(Blackboard blackboard)
            {
            ...
            {
                while (_index < _blackboard._valueLastIndex)
                {
                    ref readonly ValueSlot slot = ref _blackboard._valueSlots[_index++];
                    if (!slot.exists)
                        continue;
